...
class ActiveRecord::Base
  class << self
    ...
    def import_without_validations_or_callbacks( column_names, array_of_attributes, options={} )
      ...
      values_sql = values_sql_for_columns_and_attributes(columns, array_of_attributes, options[:encoding] || 'UTF-8')
    ...
    def values_sql_for_columns_and_attributes(columns, array_of_attributes, encoding)   # :nodoc:
        ...
        my_values = arr.each_with_index.map do |val,j|
          ...
          if val.nil? && column.name == primary_key && !sequence_name.blank?
            ...
            if serialized_attributes.include?(column.name)
              ...
              connection_memo.quote(val, column).encode(encoding)
            end
          end
        end
        "(#{my_values.join(',')})"
